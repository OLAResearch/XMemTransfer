#!/usr/bin/env bash
# §5.2 Downstream evaluation with FineWeb-Edu source + FineWeb-Edu matched adaptor.
# Reproduces Table: downstream (2 sources × 4 targets × 3 seeds = 24 runs + 24 evals).
# Prerequisites: results/slimpajama_source_{4b,9b}/memory.pt (run/02_source_memories.sh).
...
declare -A SRC_DIRS
SRC_DIRS[fw4b]="results/slimpajama_source_4b"
SRC_DIRS[fw9b]="results/slimpajama_source_9b"
...
for SRC_KEY in fw4b fw9b; do
    ...
    for i in "${!TARGETS[@]}"; do
        ...
        for SEED in 42 137 2024; do
            echo ""
            echo "==== ${SRC_KEY} -> ${TLABEL} seed=${SEED} (FW matched adaptor) ===="
...
echo ""
echo "==== FW matched downstream DONE ===="
